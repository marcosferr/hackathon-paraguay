...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ error: "Error al obtener hackathon" }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { rows: existingRows } = await sql`SELECT * FROM hackathons WHERE id = ${params.id}`
    ...
    return NextResponse.json({ error: "Error al actualizar hackathon" }, { status: 500 })
  }
}
